---
title: "Final Project: NYPD Crime Complaint Analysis"
...
First, I am de-selecting rows that I deem unnecessary to my analysis due to excessive NA values. I am also renaming some of the column names and offense descriptions to be clearer. 
```{r}
crime <- crime %>%
...
crime$date_start <- mdy(crime$date_start) 
 # Ensuring it is the correct data types and is formatted correct as dates  
str(crime$date_start)
```

The date_end column is going to require further cleaning. It has NA values throughout so I have decided to flag these missing values by creating a new column and then using a placeholder value so that I do not have to delete the entire row, since there are other important pieces of information.
```{r}
...
I've encountered a new problem — there are year typos such as 1022 and 1023 in the CMPLNT_FR_DT column. I suspect they are supposed to be 2022 and 2023, but to simplify the data and preserve data integrity, I have decided to focus my analysis on data from 2017 and onwards.
```{r}
...
There are a few more NA values I want to take care of in the suspect demographic columns. I am going to fill them in with "UNKNOWN" as that is the values for missing information in the victim information columns.
```{r}
crime <- crime %>% 
...
**Attempted crimes only make up about 1.46% of the total crime complaints.** 

## Analysis 

With a substantial amount of crime, what are the most common types in New York City? 
```{r}
crime_type <- crime %>%
...
How common is each of the types of offenses? 
```{r}
offense_freq <- crime %>%
  count(offense_level) %>%
  arrange(desc(n))
...
I am curious about the victim demographics. Who made up the largest proportion of victims since 2017? 
```{r}
vic_age <- crime %>%
...
The data contains a high count of 'UNKNOWN' values for the victim race. Excluding this result, **the most common victims of crime were individuals of black and white Hispanic races, females, and those in the 25-44 age group.** However, the data shows that females and males made up a similar proportion of crime victims -- there is approximately 9.36% difference between the two groups. 


## Investigating Suspect Demographics 

I am curious about the suspect demographics. Who made up the largest proportion of suspects since 2017?
```{r}
susp_age <- crime %>%
...
I want to visualize this on a map. Where in the city did the most crime occur? 
```{r}
crime_map <- leaflet() %>%
  addTiles() %>%
  addCircleMarkers(
...
**The point with the highest crime count (1320) is located on the border of the Garment District, just south of Times Square. Outside of Manhattan, Brooklyn and Queens had lower crime counts ranging from 530-693.** 
